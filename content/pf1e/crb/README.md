...
| `items.json` | Item **id, name, pounds**, plus documentary weapon/armor stats. Batch 10 goldens. Batches 16–19: remaining simple, martial, and exotic weapons and ammo. W1–W6: `weapon.properties` tags (`reach`, `brace`, `trip`, `disarm`, `monk`, `nonlethal`) — N tags, one or many. Batches 20–21: remaining armor and shields ([pack design §7](../../../docs/pf1e-crb-pack-design.md)). Magic weapons/armor later. Combat numbers stay on `armorClass` / `attacks`. |
...
See [`docs/pf1e-crb-pack-design.md`](../../../docs/pf1e-crb-pack-design.md) for the review process, landed batches (1–19 and W1–W6, including Batch 7 and Batch 15 hybrid Max), and the remaining mundane equipment queue (W7 then 20–21). `weapon.properties` is a **list of N tags** (one is valid; many are valid; later magic properties use the same entry). APG Summoner lives in [`../apg/`](../apg/). **Never** add `class.summoner` here.
