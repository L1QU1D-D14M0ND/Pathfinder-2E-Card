...
| `items.json` | Item **id, name, pounds**, plus documentary weapon/armor stats. Batch 10 goldens. Batch 16: remaining simple melee/ranged and simple ammo. Batch 17: martial light + remaining martial one-handed. Batch 18: martial two-handed + martial ranged and arrows. Batches 19–21: remaining exotic weapons, armor, and shields ([pack design §7](../../../docs/pf1e-crb-pack-design.md)). Magic weapons/armor later. Combat numbers stay on `armorClass` / `attacks`. |
...
See [`docs/pf1e-crb-pack-design.md`](../../../docs/pf1e-crb-pack-design.md) for the review process, landed batches (1–14 and 16–18, including Batch 7), and the remaining mundane equipment queue (19–21). After all weapon ids, Special tags land as a **multi-value** `weapon.properties` list (some weapons have two or more; later magic properties use the same entry). APG Summoner lives in [`../apg/`](../apg/). **Never** add `class.summoner` here.
