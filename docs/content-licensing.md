...
| `items.json` | Golden weapons/armor/spellbook plus Batches 16–19 simple, martial, and exotic weapons (and their ammo): pounds and documentary combat fields. W1–W7 add optional `weapon.properties` as N kebab-case tags (one or many; pack design §7.6). W7 also stamps optional `secondHead` numbers on double weapons. Batch 20 remaining light + medium armor. Remaining heavy armor and shields are queued (pack design §7); magic items are not in this folder yet |
