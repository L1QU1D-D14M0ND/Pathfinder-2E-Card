...
The next **code** increment is **Phase 3c batch 9** (Fighter/Wizard class skills + skill points). Batches 1–6 and 8 are in the repo. After 9: weapons/armor ids (10). Annotated queue: [`pf1e-crb-pack-design.md`](pf1e-crb-pack-design.md) §6. Sidebar **tools** wait until the character sheet is ~90% done. Named later: Attack Helper, Actions List, Budget Calculator.
...
| Content | `content/pf1e/crb/` batches 1–6 and 8 landed; upcoming catalog batches annotated in the pack design (§6). Next code: batch 9 class skills |
...
After goldens can be typed by hand. Review CRB character mechanics **two at a time** ([`pf1e-crb-pack-design.md`](pf1e-crb-pack-design.md) §2 and §6). Batches 1–6 (math) and 8 (Human) landed. **Next: batch 9** class skills + skill points. Then weapons/armor ids (10). Resolver: miss → custom; do not fail Load.
...
**Still sequenced (next code is batch 9 only):**

| Item | Disposition |
| --- | --- |
| Fighter/Wizard class-skill lists + skill points | Batch 9 |
| Weapon/armor catalog ids on the goldens | Batch 10 |
