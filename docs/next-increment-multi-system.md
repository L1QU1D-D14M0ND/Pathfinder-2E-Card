...
The next **code** increment is **Batch 7 pack review** (spell DC / bonus slots already in the engine). Batches 1–6 and 8–13 are in the repo. Annotated queue: [`pf1e-crb-pack-design.md`](pf1e-crb-pack-design.md) §6. **1.0** (not next code): Spanish plus a playable APG Synthesist Summoner. Sidebar **tools** wait until the character sheet is ~90% done. Named later: Attack Helper, Actions List, Budget Calculator.
...
| Content | `content/pf1e/crb/` batches 1–6 and 8–13 landed. Next code: Batch 7 pack review. 1.0: APG Synthesist |
...
After goldens can be typed by hand. Review CRB character mechanics **two at a time** ([`pf1e-crb-pack-design.md`](pf1e-crb-pack-design.md) §2 and §6). Batches 1–6 and 8–13 landed. **Next: Batch 7 pack review.** **1.0** pulls APG Synthesist Summoner (separate pack). Resolver: miss → custom; do not fail Load.
...
- [ ] Batch 7 (spell DC / bonus slots): already in engine; pack review next
...
**Still sequenced (next code is Batch 7 pack review only):**

| Item | Disposition |
| --- | --- |
| Batch 7 pack review (spell DC / bonus slots) | Next pack increment (engine already landed) |
