...
**Licensing constraint (ADR 0007) — this is a hard rule, not a style preference:** pack JSON is **mechanics-only**. No `description`/`summary`/`benefit`/`flavor`/`text`/`prose`/etc. keys, no Product Identity (Golarion place names, Paizo branding, adventure/NPC names, a short deity/place tripwire), no `source.page`, no third-party SRD scrape URLs. A test scans every pack JSON file (including `pack.json` notes) — don't add rules-text prose to a pack without first reading ADR 0007 and adding the required OGL 1.0a Section 15 notice as its own increment. `pack.json` manifests must keep `contentKind: "mechanics-only"` and `oglNoticeRequired: false` until that happens. The app `LICENSE` file is stock MIT; catalog and unofficial notices live in the README.
